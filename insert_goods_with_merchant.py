...
def create_merchants():
    ...
    with app.app_context():
        # 检查是否已有商户
        existing_merchants = Merchant.query.all()
        if existing_merchants:
            print(f"数据库中已有 {len(existing_merchants)} 个商户")
            return
        ...
        for record in merchant_records:
            merchant = Merchant(
                ...
                password=record['password'],  # 注意：实际项目中应该加密
                phone=record['phone'],
                email=record['email']
            )
            db.session.add(merchant)
            print(f"创建商户: {record['username']}")
